export class SlackAPI {
  ...
  async getUnreadMessages({ myUserId, vipUserIds = [], maxChannels = 20 } = {}) {
    ...
    // Directly finds messages mentioning the user — no channel membership required
    let searchWorked = false;
    if (myUserId) {
      try {
        const queries = [
          `<@${myUserId}> after:${afterDate}`,   // @mentions
          `to:${myUserId} after:${afterDate}`,    // DMs sent to user
        ];
    ...
    // ── Strategy 2: scan DM channels (im/mpim) — almost always accessible ───────
    // Supplement search with DMs even if search worked
    try {
      let cursor = null;
      const dmChannels = [];
      do {
  ...
  async getUserInfo(userId) {
    try {
      const data = await this.fetch('users.info', { user: userId });
      return data.user;
    } catch {
      return null;
    }
  ...
  async postDailyReport({ channel, jiraIssues, slackMessages, botName = 'WorkSync Bot' }) {
    ...
    const slackBlocks = urgentSlack.length ? [
      ...
      {
        type: 'section',
        text: { type: 'mrkdwn', text: `*💬 Important Slack Messages (${urgentSlack.length})*` },
      },
      ...urgentSlack.map(msg => ({
        type: 'section',
        text: {
          type: 'mrkdwn',
          text: `• *#${msg.channelName}* [${msg.importanceLabel}] — ${msg.text.slice(0, 100).replace(/\n/g, ' ')}${msg.url ? ` (<${msg.url}|view>)` : ''}`,
        },
      })),
    ] : [];
    ...
    const blocks = [
      ...
      { type: 'divider' },
      ...(hasItems ? [...jiraBlocks, ...slackBlocks] : [{
        type: 'section',
...
/**
 * Score a Slack message's importance based on content and context.
...
export function scoreImportance(message, myUserId, channel, vipUserIds = []) {
  ...
  // Important keywords
  const importantKeywords = ['important', 'deadline', 'due today', 'due tomorrow', 'review', 'approval', 'approve', 'sign off', 'help needed', 'need help', 'feedback'];
  ...
  // Time-sensitive: recent message (within 2 hours)
